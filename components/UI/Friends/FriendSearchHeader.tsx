...
interface Props {
 ...
 const headerHeightFriendSearchHeader = verticalScale(50)


const FriendSearchHeader: React.FC<Props> = ({
  animatedValue,
  onChange,
}) => {
  ...
  const scrollAnimation = useAnimatedStyle(() => {
    return {
      height : (interpolate(animatedValue.value, [0, 100], [headerHeightFriendSearchHeader, 0], Extrapolation.CLAMP))
    } 
  })
